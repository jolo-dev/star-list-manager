...
- `api.github.com`: required to validate identity, read public stars and native Lists, and perform only confirmed authenticated-user Starring requests or the static `UpdateUserListsForItem` operation when optional write access and the membership capability gate are enabled.

No broad page access, notification permission, private-repository permission, repository-content permission, organization permission, or remote code is requested. Optional OAuth authorization requests `public_repo user`. `public_repo` grants broader public-repository write authority than the extension uses, and `user` grants broader read/write profile authority, including email and follow subscopes. The extension implements no profile, email, or follow requests and restricts the account-matched credential to exact Starring routes and one internally constructed `UpdateUserListsForItem` document accepting only a repository node ID and complete canonical List IDs. Native List membership requires `user`; a previously stored `public_repo`-only credential remains limited to Starring.
