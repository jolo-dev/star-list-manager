...
The application will use a registered GitHub App with device flow enabled. The client ID is public and can ship with the extension. Device-flow user tokens can be refreshed without shipping a client secret. The verified core permission envelope is user-level Starring read plus implicit public-resource access and no repository or organization permission. The core exposes no mutation path.

The completed capability spike proved device authorization, secretless refresh, public star reads, and `viewer.lists` reads. It also proved that the core GitHub App token is denied REST star writes, `CreateUserList`, and `UpdateUserListsForItem`. Native-list reads remain capability-gated rather than falling back to cookie or HTML scraping, and later mutation changes are blocked until a separate write-auth proposal is verified.
...
- [The verified GitHub App permission does not authorize remote writes] -> Keep the core read-only and block later unstar and native-list mutation changes until a separate write-auth proposal is approved and verified.
