...
The system SHALL use a single GitHub App permission envelope limited to user-level Starring read/write and implicit public-resource access, with no repository or organization permission. The core change MUST NOT issue a GitHub write mutation even though the approved MVP permission envelope supports the separately specified write changes.

#### Scenario: User reviews the authorization request

- **WHEN** the user authorizes the GitHub App
- **THEN** the request identifies Starring access and does not request private repository, repository content, organization, Gist, or unrelated account permission
