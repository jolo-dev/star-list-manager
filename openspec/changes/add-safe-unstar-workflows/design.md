...
This change follows `build-star-manager-core`, which establishes background-owned GitHub access, stable repository node IDs, local annotations, IndexedDB, and synchronization. Unstarring is the first feature that intentionally changes the user's authoritative GitHub state. Extension background contexts can stop at any time, and a network failure after sending a request can leave the local client uncertain whether GitHub applied it.
...
4. Enable real REST requests only after the development GitHub App write-permission probe passes on disposable fixtures.
